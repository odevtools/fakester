...
function getFirstInternalVowel(word) {
    var vowels = word.substring(1).match(new RegExp('[AEIOU]'));
    if (vowels) {
        return vowels[0] || 'X';
    }
    else {
...
function getFirstInternalConsonant(word) {
    var vowels = word.substring(1).match(new RegExp('[BCDFGHJKLMNPQRSTUVWXYZ]'));
    if (vowels) {
        return vowels[0] || 'X';
    }
    else {
...
function formatIBAN(value) {
    return iban$1.printFormat(value);
}
function formatBBAN(value) {
    return iban$1.printFormat(value);
}
...
function removeCommonNames(name) {
    notAcceptedNames.forEach(function (notAccepted) {
        name = name.replace(new RegExp('^' + notAccepted), '');
    });
    return name;
}
function removePrefixes(name) {
    prefixes.forEach(function (notAccepted) {
        name = name.replace(new RegExp('^' + notAccepted), '');
    });
    return name;
}
...
function mapCharacterToTwoDigitsCode(c) {
    ...
    var m = map[c];
    if (!m) {
        throw Error("No two-digit code mapping for char ".concat(c));
    }
    return m;
}
function getLastRFCDigits(fullName) {
    var dictionary = '123456789ABCDEFGHIJKLMNPQRSTUVWXYZ';
    var mappedFullName = '0' + normalize(fullName)
        .split('')
...
function getRFCVerificationDigit(rfc) {
    var sum = rfc
        .split('')
        .map(function (c) { return characterValues[c.toUpperCase()] || 0; })
...
function rfc(name, surnameFather, surnameMother, bornDay, bornMonth, bornYear) {
    ...
    var rfc = getCommonPart(name, surnameFather, surnameMother, bornDay, bornMonth, bornYear, 1);
    rfc += getLastRFCDigits("".concat(name, " ").concat(surnameMother, " ").concat(surnameFather));
    rfc += getRFCVerificationDigit(rfc);
    return rfc;
}
function curp(name, surnameFather, surnameMother, bornDay, bornMonth, bornYear, bornState, gender) {
    name = clearString(name);
    ...
    bornMonth = clearString(bornMonth);
    bornYear = clearString(bornYear);
    var curp = getCommonPart(name, surnameFather, surnameMother, bornDay, bornMonth, bornYear, 0);
...
function nie() {
    var r = Math.floor(3 * Math.random()), t = Math.floor(1e7 * Math.random()), a = calculateLetter(pad(parseInt(r.toString() + pad(t.toString(), 7), 10), 8));
    return "XYZ".charAt(r) + pad(t.toString(), 7) + a;
}
function cif() {
    var r = "ABCDEFGHJNPQRSUVW";
    r = r.charAt(Math.floor(17 * Math.random()));
    var t = Math.floor(100 * Math.random()), a = Math.floor(1e5 * Math.random()), n = r + pad(t.toString(), 2) + pad(a.toString(), 5);
...
function iban() {
    return iban$1.fromBBAN('ES', bban());
}

window.ui = (function (generator) {
    var utils = {
        setupField: function (prefix, type, fn) {
            var field = document.getElementById("".concat(prefix, "-").concat(type, "-field"));
            var btn = document.getElementById("".concat(prefix, "-").concat(type, "-button"));
            if (!field || !btn) {
                return;
            }
